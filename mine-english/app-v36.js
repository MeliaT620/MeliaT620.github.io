/* Mine English v36 — minimal demo interaction layer.
   Playback is spatial: above divider = example, below divider = word. */
(function(){
  ...
  const activeLocale=zone=>{
    const scope=zone.closest('.learning-scene,.reflection-screen')||document;
    return scope.querySelector('.accent-toggle')?.dataset.accent==='UK'?'en-GB':'en-US';
  };

  function installSpatialAudio(){
    document.querySelectorAll('.audio-zone').forEach(zone=>{
      if(zone.dataset.v36Audio==='1')return;
      zone.dataset.v36Audio='1';
      let startX=0,startY=0,dragged=false,singleTimer=null;
      zone.addEventListener('pointerdown',e=>{
        if(e.target.closest('button,[data-peek],a'))return;
        startX=e.clientX;startY=e.clientY;dragged=false;
      },true);
      zone.addEventListener('pointermove',e=>{
        if(Math.hypot(e.clientX-startX,e.clientY-startY)>9)dragged=true;
      },true);

      if(zone.dataset.wordAudio){
        zone.addEventListener('click',e=>{
          if(e.target.closest('button,[data-peek],a')||dragged)return;
          e.preventDefault();e.stopPropagation();
          speak(zone.dataset.wordAudio,activeLocale(zone),.84);
        });
        return;
      }
      ...
      zone.addEventListener('click',e=>{
        ...
        singleTimer=setTimeout(()=>speak(zone.dataset.focusAudio||zone.dataset.fullAudio,activeLocale(zone),.9),230);
      });
      zone.addEventListener('dblclick',e=>{
        if(e.target.closest('button,[data-peek],a')||dragged)return;
        e.preventDefault();e.stopPropagation();
        clearTimeout(singleTimer);
        speak(zone.dataset.fullAudio||zone.dataset.focusAudio,activeLocale(zone),.92);
      });
    });
  }

  function installAccentToggles(){
    document.querySelectorAll('.accent-toggle').forEach(btn=>{
      if(btn.dataset.v36Accent==='1')return;
      btn.dataset.v36Accent='1';
  ...
  function installSocial(){
    ...
    if(!screen||!layer||screen.dataset.v36Social==='1')return false;
    screen.dataset.v36Social='1';
    const buttons={
      like:screen.querySelector('[data-social="like"]'),
      save:screen.querySelector('[data-social="save"]'),
      share:screen.querySelector('[data-social="share"]'),
      comment:screen.querySelector('[data-social="comment"]')
    };
    ...
    const comments=[
      ['原来 lead 两个读音完全不一样',32],
      ['made of lead 整块记，更容易想起来',18],
      ['先看句子，再看词义，这个顺序很舒服',27],
      ['一个 Usage 一段记忆历史，很直观',41],
      ['这种页面很安静，我会愿意多看一会儿',23]
    ];
    let cursor=0;
    const spawn=()=>{
      if(layer.classList.contains('is-hidden'))return;
      const [text,baseLikes]=comments[cursor++%comments.length];
      const item=document.createElement('div');
      item.className='live-comment';
      item.innerHTML=`<div class="live-comment-body"><span class="live-comment-text"></span><button class="comment-like" type="button" aria-pressed="false">${heart}<span>${baseLikes}</span></button></div>`;
      ...
      like.addEventListener('click',e=>{
        ...
        like.querySelector('span').textContent=String(baseLikes+(active?1:0));
      });
      layer.appendChild(item);
      setTimeout(()=>item.remove(),7400);
    };
    spawn();setTimeout(spawn,1900);screen._v36Comments=setInterval(spawn,2250);
    return true;
  }

  function boot(){
    installSpatialAudio();installAccentToggles();
    if(!installSocial()){
      const t=setInterval(()=>{installSpatialAudio();installAccentToggles();if(installSocial())clearInterval(t)},80);
      setTimeout(()=>clearInterval(t),5000);
    }
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  new MutationObserver(()=>{installSpatialAudio();installAccentToggles()}).observe(document.documentElement,{childList:true,subtree:true});
})();